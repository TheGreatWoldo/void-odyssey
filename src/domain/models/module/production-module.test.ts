...
import { ProductionModuleLifecycleState } from './production-module-lifecycle-state';
import { UpgradeLifecycleState, UpgradeType } from './production-module-upgrade';

function makeModule(...args: Parameters<typeof createProductionModule>) {
  const result = createProductionModule(...args);
...
describe('createProductionModule', () => {
  describe('validation', () => {
    it('throws when id is blank', () => {
      expect(createProductionModule('', 'Module', powerRecipe, { type: ModuleId.ReactorCore }).ok).toBe(false);
      expect(createProductionModule('  ', 'Module', powerRecipe, { type: ModuleId.ReactorCore }).ok).toBe(false);
    });

    it('throws when name is blank', () => {
      expect(createProductionModule('m1', '', powerRecipe, { type: ModuleId.ReactorCore }).ok).toBe(false);
      expect(createProductionModule('m1', '  ', powerRecipe, { type: ModuleId.ReactorCore }).ok).toBe(false);
    });

    it('throws when condition is below 0', () => {
      expect(createProductionModule('m1', 'Module', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: -0.1 }).ok).toBe(false);
    });

    it('throws when condition is above 1', () => {
      expect(createProductionModule('m1', 'Module', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 1.1 }).ok).toBe(false);
    });

    it('accepts condition = 0 and condition = 1', () => {
      expect(createProductionModule('m1', 'M', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 0 }).ok).toBe(true);
      expect(createProductionModule('m2', 'M', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 1 }).ok).toBe(true);
    });

    it('throws when rampRate is negative', () => {
      expect(createProductionModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, rampRate: -1 }).ok).toBe(false);
    });
  });

  describe('initial state', () => {
    it('exposes id, name, and condition', () => {
      const m = makeModule('reactor', 'Reactor', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 0.8 });
    ...
    it('defaults enabled to true', () => {
      expect(makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore }).enabled).toBe(true);
    });
  });

  describe('isOperational', () => {
    it('returns true when condition > 0', () => {
      expect(makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 0.1 }).isOperational()).toBe(true);
    });

    it('returns false when condition = 0', () => {
      expect(makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 0 }).isOperational()).toBe(false);
    });
  ...
  describe('setThrottle', () => {
    ...
    describe('snapOutputToInteger', () => {
      ...
      it('snaps throttle to nearest integer output (0.6 → 0.5, maxN=2)', () => {
        const m = makeModule('m', 'M', scanRecipe, { type: ModuleId.SensorArray, snapOutputToInteger: true, maxOutput: 2 });
        m.setThrottle(0.6);
      ...
      it('snaps to maxN/maxN=1.0 when maxOutput is non-integer (e.g. 2.5 → maxN=2)', () => {
        const m = makeModule('m', 'M', scanRecipe, { type: ModuleId.SensorArray, snapOutputToInteger: true, maxOutput: 2.5 });
        m.setThrottle(0.9);
      ...
      it('does not snap when snapOutputToInteger is false', () => {
        const m = makeModule('m', 'M', scanRecipe, { type: ModuleId.SensorArray });
        m.setThrottle(0.6);
      ...
      it('falls back to normal clamp when maxOutput < 1', () => {
        const m = makeModule('m', 'M', scanRecipe, { type: ModuleId.SensorArray, snapOutputToInteger: true, maxOutput: 0.5 });
        m.setThrottle(0.6);
  ...
  describe('stepRamp', () => {
    it('with Infinity rampRate, snaps actualThrottle to throttle immediately', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, rampRate: Infinity });
      m.setThrottle(0.3);
      m.stepRamp(1);
    ...
    it('with finite rampRate, advances actualThrottle by rampRate × deltaTime', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, rampRate: 0.5 });
      m.setThrottle(0);
      m.stepRamp(1);
    ...
    it('does not overshoot the target throttle', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, rampRate: 10 });
      m.setThrottle(0);
      m.stepRamp(1);
    ...
    it('uses warm-up duration when ramping up', () => {
      const m = makeModule('m', 'M', powerRecipe, {
      ...
      m.disable();
      m.stepRamp(1); // cool to 0 instantly via Infinity fallback
      ...
      expect(m.actualThrottle).toBeCloseTo(0.5);
      expect(m.lifecycleState).toBe(ProductionModuleLifecycleState.WarmingUp);
    });

    it('uses cool-down duration when disabling', () => {
      const m = makeModule('m', 'M', powerRecipe, {
        type: ModuleId.ReactorCore,
        rampRate: Infinity,
        coolDownSeconds: 2,
      ...
      expect(m.actualThrottle).toBeCloseTo(0.5);
      expect(m.lifecycleState).toBe(ProductionModuleLifecycleState.CoolingDown);
    });

    it('degrades condition over time when operating under load', () => {
      const m = makeModule('m', 'M', powerRecipe, {
        type: ModuleId.ReactorCore,
        degradationPerSecond: 0.1,
      });

      m.stepRamp(2);

      expect(m.condition).toBeCloseTo(0.8);
    });

    it('transitions to Failed when wear reaches zero condition', () => {
      const m = makeModule('m', 'M', powerRecipe, {
        type: ModuleId.ReactorCore,
        initialCondition: 0.2,
        degradationPerSecond: 1,
      });

      m.stepRamp(1);

      expect(m.condition).toBe(0);
      expect(m.lifecycleState).toBe(ProductionModuleLifecycleState.Failed);
      expect(m.enabled).toBe(false);
    });
  });
  ...
  describe('setCondition', () => {
    it('throws when value is below 0', () => {
      ...
      expect(m.setCondition(-0.1).ok).toBe(false);
    });

    it('throws when value is above 1', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });

      expect(m.setCondition(1.1).ok).toBe(false);
    });

    it('accepts boundary values 0 and 1', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });

      expect(m.setCondition(0).ok).toBe(true);
      expect(m.setCondition(1).ok).toBe(true);
    });
  });
  ...
  describe('addUpgrade', () => {
    ...
    it('rejects duplicate upgrade id on the same module', () => {
      ...
      expect(m.upgrades[0].name).toBe('U1');
    });
  });
  ...
  describe('enableUpgrade / disableUpgrade', () => {
    ...
    it('enables an installed upgrade', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });
      m.addUpgrade({ id: 'u1', name: 'U1', type: UpgradeType.Efficiency, costFactor: 0.8, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: false });
      m.enableUpgrade('u1');
    ...
    it('disables an installed upgrade', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });
      m.addUpgrade({ id: 'u1', name: 'U1', type: UpgradeType.Efficiency, costFactor: 0.8, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: true });
      m.disableUpgrade('u1');

      expect(m.upgrades[0].enabled).toBe(false);
      expect(m.upgrades[0].lifecycleState).toBe(UpgradeLifecycleState.Disabled);
    });

    it('supports explicit install progression before activation', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });

      m.startUpgradeInstall({
        id: 'u1',
        name: 'U1',
        type: UpgradeType.Efficiency,
        costFactor: 0.8,
      ...
      expect(m.upgrades[0].lifecycleState).toBe(UpgradeLifecycleState.Installing);

      m.completeUpgradeInstall('u1');
      expect(m.upgrades[0].lifecycleState).toBe(UpgradeLifecycleState.Installed);

      m.enableUpgrade('u1');
      expect(m.upgrades[0].lifecycleState).toBe(UpgradeLifecycleState.Active);
    });
  });
  ...
  describe('costMultiplier', () => {
    ...
    it('stacks multiple upgrade costFactors additively', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });
      m.addUpgrade({ id: 'u1', name: 'U1', type: UpgradeType.Efficiency, costFactor: 0.8, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: true });
      m.addUpgrade({ id: 'u2', name: 'U2', type: UpgradeType.Power, costFactor: 0.7, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: true });
    ...
    it('skips disabled upgrades', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });
      m.addUpgrade({ id: 'u1', name: 'U1', type: UpgradeType.Efficiency, costFactor: 0.5, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: false });
    ...
    it('clamps to 0 when additive reductions drive the multiplier below zero', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore });
      m.addUpgrade({ id: 'u1', name: 'U1', type: UpgradeType.Efficiency, costFactor: 0.3, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: true });
      m.addUpgrade({ id: 'u2', name: 'U2', type: UpgradeType.Power, costFactor: 0.3, targetResourceType: primaryOutputType, storableType: 'upgrade' as const, slotCost: 1, enabled: true });
  ...
  describe('produce', () => {
    ...
    it('does not accumulate output when condition is 0', () => {
      const m = makeModule('m', 'M', powerRecipe, { type: ModuleId.ReactorCore, initialCondition: 0 });
      const sources = makeContainerMap([ResourceType.Fuel]);
      const targets = makeContainerMap([ResourceType.Power]);
